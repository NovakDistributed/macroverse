...
module.exports = async function(deployer, network, accounts) {
  ...
  // On Rinkeby we deployed an old registry so we need to change it again
  if (network == "rinkeby_infura") {
    deployer.link(MacroverseNFTUtils, MacroverseUniversalRegistry)

    // On testnet we use the token that lets anyone mint tokens for free
    let token_contract = TestnetMRVToken
