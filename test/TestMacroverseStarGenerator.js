...
contract('MacroverseStarGenerator', function(accounts) {
  it("should initially reject queries", async function() {
    ...
    await instance.getGalaxyDensity.call(0, 0, 0, {from: accounts[1]}).catch(async function () {
      failure_found = true
    })
  ...
  it("should report 0.9 density out to a radius of 500", async function() {
    let instance = await MacroverseStarGenerator.deployed()
    let density = mv.fromReal(await instance.getGalaxyDensity.call(999, 0, 0))
    ...
    density = mv.fromReal(await instance.getGalaxyDensity.call(250, 866, 433))
    assert.isAbove(density, 0.899999, "Density just inside the central bubble is not too small")
    assert.isBelow(density, 0.900001, "Density just inside the central bubble is not too big")
    ...
    density = mv.fromReal(await instance.getGalaxyDensity.call(35, 872, 488))
    assert.isAbove(density, 0.899999, "Density just inside the central bubble is not too small")
    assert.isBelow(density, 0.900001, "Density just inside the central bubble is not too big")
    
    density = mv.fromReal(await instance.getGalaxyDensity.call(131, 0, 0))
    ...
    density = mv.fromReal(await instance.getGalaxyDensity.call(35, 872, 489))
    assert.isBelow(density, 0.899999, "Density just outside the central bubble is smaller")
    
    density = mv.fromReal(await instance.getGalaxyDensity.call(0, 1000, 0))
  ...
  it("should report 1/60 density way out", async function() {
    let instance = await MacroverseStarGenerator.deployed()
    let density = mv.fromReal(await instance.getGalaxyDensity.call(999, 999, 999))
  ...
  it("should report 1/2 density in the main disk", async function() {
    let instance = await MacroverseStarGenerator.deployed()
    let density = mv.fromReal(await instance.getGalaxyDensity.call(-6000, -5, -13))
  ...
  it("should produce stars of reasonable mass", async function() {
    ...
    let objMass = mv.fromReal(await instance.getObjectMass.call(seed, objClass, objType))
  ...
  it("should let us scan sector 0", async function() {
    ...
    for (let star = 0; star < starCount; star++) {
      ...
      starPromises.push(async function() {
        ...
        console.log("Star " + star + " at " + x + "," + y + "," + z + " ly is a " + mv.objectClasses[objClass] + " " + mv.spectralTypes[objType] + " of " + objMass + " solar masses" + (hasPlanets ? " with planets" : ""))
