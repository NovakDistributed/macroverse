...
contract('MacroverseSystemGenerator', function(accounts) {
  ...
  it("should have an orbit from about 0.32 to 0.38 AU", async function() {
    let instance = await MacroverseSystemGenerator.deployed()
    let planetSeed = await instance.getPlanetSeed.call('fred', 0)
    let planetClassNum = mv.planetClass['Terrestrial']
    ...
    let [realPeriapsis, realApoapsis, realClearance] = await instance.getPlanetOrbitDimensions.call(parentClassNum, parentTypeNum,
    ...
    assert.isAbove(mv.fromReal(realApoapsis) / mv.AU, 0.37)
    assert.isBelow(mv.fromReal(realApoapsis) / mv.AU, 0.38)
  ...
  it("should have a semimajor axis of 0.35 AU and an eccentricity of 0.08", async function() {
    ...
    assert.isAbove(mv.fromReal(realSemimajor) / mv.AU, 0.34)
    assert.isBelow(mv.fromReal(realSemimajor) / mv.AU, 0.36)
    
    assert.isAbove(mv.fromReal(realEccentricity), 0.07)
  ...
  it("should let us dump the whole system", async function() {
    ...
    for (let i = 0; i < count; i++) {
      ...
      let [realPeriapsis, realApoapsis, newClearance] = await instance.getPlanetOrbitDimensions.call(parentClassNum, parentTypeNum,
